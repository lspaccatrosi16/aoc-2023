...
func Challenge(input string) string {
		...
		if strings.Contains(l, ":") {
			...
			if strings.HasPrefix(l, "seeds:") {
				...
				for _, n := range nums {
					...
					iNums = append(iNums, vInt)
				}

				for i := 0; i < len(iNums); i += 2 {
					start, rLen := iNums[i], iNums[i+1]
					seeds = append(seeds, SeedsMap{start, rLen})
				}
			} else {
				parts := strings.Split(l, ":")
	...
	for i, seed := range seeds {
		fmt.Printf("Seed Range %d\n", i+1)
		for i := 0; i < seed.Len; i++ {
			loc := followPath(overallMap, path, seed.Start+i)
			if loc < minLoc {
				minLoc = loc
			}
		}
