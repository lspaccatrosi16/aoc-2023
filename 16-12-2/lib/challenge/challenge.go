...
func Challenge(input string) string {
	...
	maxX := len(grid.GetCols())
	maxY := len(grid.GetRows())

	for x := 0; x < maxX; x++ {
		vm = map[visitedPoint]bool{}
		masterBeam.Navigate(South, cartesian.Coordinate{x, 0})
		energised := len(masterBeam.EnergisedSquares())
		if energised > maxEnergised {
			maxEnergised = energised
		}
		vm = map[visitedPoint]bool{}
		masterBeam.Navigate(North, cartesian.Coordinate{x, maxY - 1})
		energised = len(masterBeam.EnergisedSquares())
		if energised > maxEnergised {
			maxEnergised = energised
		}
	...
	for y := 0; y < maxY; y++ {
		vm = map[visitedPoint]bool{}
		masterBeam.Navigate(East, cartesian.Coordinate{0, y})
		energised := len(masterBeam.EnergisedSquares())
		// fmt.Println("start at", 0, y, energised)
		if energised > maxEnergised {
			maxEnergised = energised
		}
		vm = map[visitedPoint]bool{}
		masterBeam.Navigate(West, cartesian.Coordinate{y, maxX - 1})
		energised = len(masterBeam.EnergisedSquares())
		if energised > maxEnergised {
			maxEnergised = energised
		}
